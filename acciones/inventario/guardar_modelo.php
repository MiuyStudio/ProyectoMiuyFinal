...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($id_marca) && !empty($nombre_modelo) && preg_match('/[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ]/', $nombre_modelo)) {
        ...
        if ($conn->query($sql)) {
            ...
            header("Location: ../../modulos/inventario/agregar_modelo.php?error=" . urlencode("Error al guardar el modelo: " . $conn->error));
            exit();
        }
    } else {
        header("Location: ../../modulos/inventario/agregar_modelo.php?error=" . urlencode("El nombre del modelo debe contener al menos letras o números válidos."));
        exit();
    }
} else {
    header("Location: ../../modulos/inventario/agregar_modelo.php");
    exit();
}
?>
